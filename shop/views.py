...
def countdown_view(request, event_id):
    ...
    time_remaining = event.event_time - datetime.now()
        # You can pass seconds or a formatted string to the template
    context = {'event': event, 'time_remaining_seconds': time_remaining.total_seconds()}
    return render(request, 'countdown_template.html', context)

def contact(request):
	return render(request, 'shop/shop/contact.html')
